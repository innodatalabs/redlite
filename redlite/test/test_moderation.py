...
def test_moderation_with_safety_identifier():
    """Test that safety_identifier is injected into OpenAI models"""
    inner_model = Mock()
    inner_model.name = "gpt-4"
    inner_model.return_value = "Safe response"
    inner_model.params = {}  # Simulate OpenAI model with params attribute
    ...
    assert result == "Safe response"
    # Verify params were restored after call
    assert "safety_identifier" not in inner_model.params


def test_moderation_without_safety_identifier():
    """Test that models without params attribute work normally"""
    inner_model = Mock()
    inner_model.name = "claude"
    inner_model.return_value = "Safe response"
    # No params attribute - simulates non-OpenAI model
